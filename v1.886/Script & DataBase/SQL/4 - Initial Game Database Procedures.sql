...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'LOAD_WEB_ITEMMALL') AND type in (N'P', N'PC'))
    DROP PROCEDURE LOAD_WEB_ITEMMALL;
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'SHOPPINGMALL_BUY') AND type in (N'P', N'PC'))
    DROP PROCEDURE SHOPPINGMALL_BUY;
GO

CREATE PROCEDURE SHOPPINGMALL_BUY
...
BEGIN TRAN        
        ...
        DECLARE @ItemName varchar(255),@Price int
        SELECT @ItemName = strName FROM ITEM WHERE Num = @nItemID

        INSERT INTO WEB_ITEMMALL  (strAccountID, strCharID, ServerNo, ItemID, ItemCount, nExpirationTime  ) 
        VALUES (@strAccountID, @strCharID, 0, @nItemID, @sCount, @nExpireTime )
        ...
        ELSE
        BEGIN
                INSERT INTO WEB_ITEMMALL_LOG  (strAccountID, strCharID, ServerNo, ItemID, ItemCount, nExpirationTime, BuyTime, img_file_name, strItemName, price, pay_type ) 
                VALUES (RTRIM(LTRIM(@strAccountID)), @strCharID, 0, @nItemID, @sCount, @nExpireTime, GETDATE(),STR(@nItemID) + '.gif',@ItemName,@Price,0)
        END
...
SELECT strAccountID, ItemID, ItemCount, nExpirationTime, BuyTime FROM WEB_ITEMMALL WHERE strCharID = @strCharID
-- Deleting Record
