...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'K_OBJECTPOS')
    DROP TABLE K_OBJECTPOS;
...
IF NOT EXISTS(SELECT * FROM sys.columns WHERE NAME = N'strItemEx' AND Object_ID = OBJECT_ID(N'USERDATA'))
        ALTER TABLE USERDATA ADD strItemEx binary(365) NULL
IF NOT EXISTS(SELECT * FROM sys.columns WHERE NAME = N'WarehouseDataEx' AND Object_ID = OBJECT_ID(N'WAREHOUSE'))
        ALTER TABLE WAREHOUSE ADD WarehouseDataEx binary(960) NULL
IF NOT EXISTS(SELECT * FROM sys.columns WHERE NAME = N'nExpirationTime' AND Object_ID = OBJECT_ID(N'WEB_ITEMMALL'))
        ALTER TABLE WEB_ITEMMALL ADD nExpirationTime int NOT NULL DEFAULT 0
IF NOT EXISTS(SELECT * FROM sys.columns WHERE NAME = N'nExpirationTime' AND Object_ID = OBJECT_ID(N'WEB_ITEMMALL_LOG'))
        ALTER TABLE WEB_ITEMMALL_LOG ADD nExpirationTime int NOT NULL DEFAULT 0
GO
...
CREATE TABLE K_OBJECTPOS (
        [ZoneID] [smallint] NOT NULL,
        [Belong] [tinyint] NOT NULL,
        [sIndex] [smallint] NOT NULL,
        [Type] [tinyint] NOT NULL,
        [ControlNpcID] [smallint] NOT NULL,
